...
## Phase 15: Pool-Based Dependency Model — Deadlock & Bottleneck Redesign

**Goal**: Replace the 1-to-1 dependency model with a pool model (one source, many targets). A
dependency is satisfied when ≥ 1 pool member is present; a deadlock occurs when ALL pool members
are simultaneously absent. Remove at-risk row coloring and bottleneck-absent cell coloring.
Add deadlock CW-band coloring + "Deadlock" label on the dependent person's row. Highlight phase
banners that overlap any deadlock week. Bottleneck weight replaces the old 2-incoming threshold.

**Independent Test**: Define dependency Alice → pool {Bob, Carol}. Mark Bob absent in CW24,
Carol absent in CW24 (deadlock), Carol absent in CW25 (Bob present, no deadlock). Verify: CW24
cells on Alice's row show deadlock color + "Deadlock" text; CW25 shows nothing on Alice's row;
any phase spanning CW24 turns alert color; Bob has bottleneck weight in CW25 (sole satisfier).

- [x] T084 Write failing unit tests for pool-based `absence_dashboard/graph.py` in `tests/unit/test_graph.py`: replace `TestAddEdge`, `TestRemoveEdge`, `TestGetBottlenecks`, `TestComputeAtRiskWeeks`, `TestDateBoundedDependencies` with new `TestPoolDependency` class — (a) `add_dependency` with single pool member accepted; (b) `add_dependency` with multi-member pool accepted; (c) duplicate (from_member, frozenset(to_members), active_from, active_to) rejected; (d) unknown member in to_members raises `ValueError`; (e) `remove_dependency` with matching tuple removes correct entry; (f) `remove_dependency` non-existent raises `KeyError`; (g) `compute_deadlock_weeks` returns CW when ALL pool members absent that week; (h) `compute_deadlock_weeks` returns no CW when at least one pool member present; (i) `compute_deadlock_weeks` respects `active_from`/`active_to` range; (j) `compute_bottleneck_weights` returns member → weight dict with weight ≥ 1 when member is sole present satisfier; (k) `compute_bottleneck_weights` returns 0 weight when multiple pool members present; (l) `compute_bottleneck_weights` accumulates weight across multiple dependencies
- [x] T085 Rewrite `absence_dashboard/graph.py` for pool-based model — remove `add_edge`, `remove_edge`, `get_bottlenecks`, `compute_at_risk_weeks`, `_has_cycle_if_added`, `_can_reach`; add `add_dependency(from_member: str, to_members: list[str], valid_members: set, active_from: str = None, active_to: str = None) → None` (validates all members exist, validates both-or-neither dates, validates `active_from <= active_to`, uniqueness on `(from_member, frozenset(to_members), active_from, active_to)`); add `remove_dependency(from_member: str, to_members: list[str], active_from: str = None, active_to: str = None) → None`; add `compute_deadlock_weeks(from_member: str, deps: list, member_absence_date_sets: dict, calendar_weeks: list) → list[int]` (for each active dep where from_member matches, check if ALL to_members have at least one absent day in the CW; return sorted CW numbers); add `compute_bottleneck_weights(deps: list, member_absence_date_sets: dict, calendar_weeks: list) → dict[str, int]` (for each active dep per CW, identify pool members present; if exactly one is present, increment their weight; return dict member → total weight summed across all CWs) — confirm T084 now passes
- [x] T086 [P] Write failing integration tests for pool-based dependency endpoints in `tests/integration/test_app.py`: `POST /api/dependencies` with `to_members: ["Bob", "Carol"]` returns 201; 400 on empty `to_members`; 409 on identical (from, to_members, dates) tuple; `DELETE /api/dependencies` with `to_members` list removes correct entry; `PUT /api/dependencies` with `old_to_members`/`new_to_members` lists replaces correctly; `GET /api/dashboard` member object has `deadlock_weeks` (not `at_risk_weeks`); `GET /api/dashboard` member with all pool members absent in a CW has that CW in `deadlock_weeks`; member with partial pool absence has no `deadlock_weeks` for that CW; `is_bottleneck` true when member is sole present satisfier in any CW
- [x] T087 [P] Update `absence_dashboard/static/style.css`: add `--color-deadlock: #8b0000` and `--color-phase-deadlock: rgba(192, 57, 43, 0.55)` to `:root`; add `.tg-day-cell.deadlock` class (`background: var(--color-deadlock); color: #fff; font-size: 7px; font-weight: 700; overflow: hidden`); add `.tg-row.tg-phase-row.phase-has-deadlock .tg-day-cell.phase-active` override (`background: var(--color-phase-deadlock); opacity: 1`); keep `.at-risk` and `.bottleneck-absent` classes in file (do not delete) but they will no longer be applied by JS
- [x] T088 Update `absence_dashboard/app.py` — (a) replace `to_member` with `to_members` (list) throughout all dependency routes and `GET /api/dashboard`; (b) update stale-reference check in `POST /api/refresh` to iterate `dep["to_members"]` list; (c) replace `get_bottlenecks` + `compute_at_risk_weeks` calls in dashboard assembly with `compute_bottleneck_weights` + `compute_deadlock_weeks`; build `member_absence_date_sets: dict[str, set[date]]` from merged blocks before calling these methods; populate `deadlock_weeks` (replaces `at_risk_weeks`) and `is_bottleneck` (true if member's total bottleneck weight > 0) per member in the response; remove `depends_on` and `bottlenecks` top-level keys from response — confirm T086 now passes
- [x] T089 Update `absence_dashboard/static/main.js` — (a) rewrite `computeDayClasses`: remove `atRiskWeeks` parameter and `at-risk` / `bottleneck-absent` logic; add `deadlockWeeks` parameter; if `deadlockWeeks.includes(week)` apply class `deadlock` to all 5 cells of that week; insert `<span class="deadlock-label">Deadlock</span>` in the first cell of each deadlock week; (b) update member row rendering to call `computeDayClasses(member.merged_blocks, dayIndex, member.deadlock_weeks)`; (c) update phase banner rendering: before rendering phase cells compute `const deadlockWeekSet = new Set(data.members.flatMap(m => m.deadlock_weeks || []))`; if any day in the phase's date range falls in a deadlock CW add class `phase-has-deadlock` to the phase row; (d) update `renderDependencies`: display shows `dep.to_members.join(", ")`; edit form "To" becomes a multi-`<select>`; `POST` body uses `to_members: Array.from(newToSel.selectedOptions).map(o=>o.value)`; `DELETE` body uses `to_members: dep.to_members`; `PUT` body uses `old_to_members`/`new_to_members` arrays; (e) update Add Dependency button handler to read multi-select value as array
- [x] T090 [P] Update `absence_dashboard/static/index.html` dependency panel: replace `<select id="dep-to">` with `<select id="dep-to" multiple size="4">` and update its label to "To pool (one or more)"; keep all other panel markup unchanged

**Checkpoint**: Pool-based dependencies functional. Deadlock CW bands highlighted on dependent's
row with "Deadlock" label. Phase banners alert-colored on deadlock overlap. Bottleneck badge
reflects sole-satisfier weight. No at-risk or bottleneck-absent cell coloring remains.
